module headLines {
	 ...
	 *
	 */
	export class HeadLines extends eui.Component{
          public scor1:eui.Scroller;     
          private img_lookMore:eui.Image;//显示更多
          private group_itemData:eui.Group;//添加项
          private lable_starNews:eui.Label;//没有图片
          private obj:Object;
          private img_btnBack:eui.Image;
          private group_data:eui.Group;//显示多长时间
          private img_btnAdd:eui.Image;//添加按钮
          private grou_task:eui.Group;//添加新闻
          private rect_btnLookMore:eui.Rect;//显示更多；
          public headline_id:string;
          private isUpdateRank:Boolean = false;
          private type:number;  //1进入主页 2进入本页
          
          private lable_addNews:eui.Label;
          ...
          private text_title:eui.EditableText;
          private text_taskDirections:eui.EditableText;
          private lable_warmTitle:eui.Label;
          private rect_button:eui.Rect;
          private lable_guizhe:eui.Label;
          
          private arr: Array<headLines.HeadLinesItem> = []
          public index:number;
          private disNum:number = 3;
       
          public num:number;//用来判断是本周还是以前的日期
          public static timeCd:number;
          public static timeArr:Array<number> = [];
          public n:number;
          public static toDayTime:number;
          public static isAdd:boolean = true;
...
//            this.scor1.addEventListener(eui.UIEvent.CHANGE,this.change,this);
       ...
       public deletweeTime(e:egret.Event):void{
           console.log("scrol----");
           this.scor1.removeEventListener(egret.TouchEvent.TOUCH_MOVE,this.deletweeTime,this);
           this.img_lookMore.scaleY = 1;
           var item: mySelfStar.MySelfHeadLinesItem = <mySelfStar.MySelfHeadLinesItem>this.group_data.getElementAt(0);
        ...
        public addNews(e:egret.Event):void{
            var self: HeadLines = this;
            if((this.text_taskDirections.text == "")) {
                this.lable_warmTitle.visible = true;
                return;
            }
            GameApp.Manager.viewManager.maskStage(.4,true);//加loading遮照
            var sign: string = new md5().hex_md5(GameApp.Manager.dataManager.uid + GameApp.Manager.dataManager.bid + GameApp.Manager.dataManager.group_openid + window["decToHex"](this.text_title.text) + this.text_taskDirections.text + "&");
            GameApp.Manager.viewManager.maskStage(.4,true);//加loading遮照
            Util.sendJson1(GameApp.Manager.dataManager.IP + "/userHeadlineSuggest",{ 
                ...
                "title": window["decToHex"](this.text_title.text),"link": this.text_taskDirections.text,sign: sign },function(obj: Object) {
                    if(obj["st"] == 1) {
       ...
       public addStarNews(e:egret.Event):void{
           this.type = 2;
        ...
        public selectWeekTime(e:egret.Event):void{
            var self: HeadLines = this;
            ...
            if(this.img_lookMore.scaleY == 1){
                ...
                for(var i: number = 0;i < HeadLines.timeArr.length;i++) {
                    if(HeadLines.timeArr[i] == HeadLines.timeCd ) {           //如果是第二种情况
                        var item: mySelfStar.MySelfHeadLinesItem = <mySelfStar.MySelfHeadLinesItem>this.group_data.getElementAt(0);
                        item.rect_button.fillColor = 0xEBEBEB;
                        console.log("相同的");
                    }else {
                        console.log("不相同");
                        var data_weekTime: mySelfStar.MySelfHeadLinesItem = mySelfStar.MySelfHeadLinesItem.produce();
        ...
        public setData(obj:Object,comeType:number,dis:number,weekTime:number):void{
...
//                console.log("1");
                ...
                if(HeadLines.isAdd){    //每次登陆进去只能添加一次
                    HeadLines.isAdd = false
                    obj["historyTs"].push(obj["currentTs"]);
                    for(var i: number = 0;i < obj["historyTs"].length;i++) {
                        var ti: number = <number>obj["historyTs"][i];
                ...
                if(obj["headline"].length == 0) {
                    ...
                    for(var i: number = 0;i < obj["headline"].length;i++) {
                        var it = obj["headline"][i];
                        var headItem = headLines.HeadLinesItem.produce();
                        headItem.x = 0; headItem.y = i * headItem.height;
...
//            }
//        }
        public clickWeekTime(e:egret.Event):void{
            var self: HeadLines = this;
            var item: mySelfStar.MySelfHeadLinesItem = e.data;
            self.img_lookMore.scaleY = 1;
            
            HeadLines.ts = item.time?item.time:HeadLines.toDayTime;
            
//            console.log("HeadLines.ts",HeadLines.ts)
            if(item.time == headLines.HeadLines.timeCd){   //如果是自己的时间的话
//                console.log("是自己的时间")
                item.rect_button.fillColor = 0xffffff;
                item.rect_button.strokeColor = 0xffffff;
                var num = self.group_data.numElements;
...
//                    console.log("往期时间");
                ...
                while(this.group_data.numElements>0){
                    var ite: mySelfStar.MySelfHeadLinesItem = <mySelfStar.MySelfHeadLinesItem>this.group_data.getElementAt(this.group_data.numElements - 1);
                    mySelfStar.MySelfHeadLinesItem.reclaim(ite);
                    if(ite.parent) {
                        ite.parent.removeChild(ite);
                    }
        ...
        private votesUPItem(e:egret.Event): void {
            var n: number = 0;
            var self: headLines.HeadLines = this;
            var item: HeadLinesItem = e.data;
            
            var tip: system.TipText1 = system.TipText1.produce("1");
            tip.x = item.lable_count.x + 68; tip.y = item.lable_count.y - 25;
            ...
            for(var i:number = 0;i<this.group_itemData.numElements;i++){
                var it: headLines.HeadLinesItem = <headLines.HeadLinesItem>this.group_itemData.getElementAt(i);
                if(item == it) 
                    break;
...
//                GameApp.Manager.controllerManager.topGroupRankController.show();
        ...
        public clear():void{
            ...
            while(this.group_itemData.numElements>0){
                var item: headLines.HeadLinesItem = <headLines.HeadLinesItem>this.group_itemData.getElementAt(this.group_itemData.numElements -1);
...
//            
        ...
        public deleteTimeItem():void{
            this.img_lookMore.scaleY = 1;
            while(this.group_data.numElements > 0) {
                var item: mySelfStar.MySelfHeadLinesItem = <mySelfStar.MySelfHeadLinesItem>this.group_data.getElementAt(this.group_data.numElements - 1);
